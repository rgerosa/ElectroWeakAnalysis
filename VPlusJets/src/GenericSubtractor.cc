...
#include "ElectroWeakAnalysis/VPlusJets/interface/SimpleGhostRescaler.hh"
#include "ElectroWeakAnalysis/VPlusJets/interface/ShapeWithPartition.hh"
...
double GenericSubtractor::operator()(const fastjet::FunctionOfPseudoJet<double> &shape,
...
// the action on a given jet for a given shape
double GenericSubtractor::operator()(const fastjet::FunctionOfPseudoJet<double> &shape,
                                      const fastjet::PseudoJet &jet, GenericSubtractorInfo &info) const{
  // make sure we have a BGE or a rho value
  if (!_bge_rho && !_externally_supplied_rho_rhom)
    throw fastjet::Error("GenericSubtractor::operator(): generic subtraction needs a JetMedianBackgroundEstimator or a value for rho");
  ...
  fastjet::PseudoJet working_jet = (shape_with_partition_ptr != 0)
  ...
  vector<fastjet::PseudoJet> ghosts = fastjet::SelectorIsPureGhost()(working_jet.constituents());
  if (ghosts.size() == 0){ 
  ...
  for (vector<fastjet::PseudoJet>::iterator git=ghosts.begin(); git!=ghosts.end();git++) {
  ...
  if ( _externally_supplied_rho_rhom ) {
    ...
    if (_bge_rhom){
      ...
      fastjet::BackgroundJetPtMDensity _m_density;
      fastjet::JetMedianBackgroundEstimator *jmbge = dynamic_cast<fastjet::JetMedianBackgroundEstimator*>(_bge_rho);
      const fastjet::FunctionOfPseudoJet<double> * orig_density = jmbge->jet_density_class();
...
void GenericSubtractor::use_common_bge_for_rho_and_rhom(bool value){ 
  if (value){
    ...
    if (_bge_rhom)
      throw fastjet::Error("GenericSubtractor::use_common_bge_for_rho_and_rhom() is not allowed in the presence of an existing background estimator for rho_m.");

    // make sure we are not using externally supplied values for tho and rhom
    if (_externally_supplied_rho_rhom)
      throw fastjet::Error("GenericSubtractor::use_common_bge_for_rho_and_rhom() is not allowed when supplying externally the values for rho and rho_m.");

    // check the background estimator type
    fastjet::JetMedianBackgroundEstimator *jmbge = dynamic_cast<fastjet::JetMedianBackgroundEstimator*>(_bge_rho);
    if (!jmbge)
      throw fastjet::Error("GenericSubtractor::use_common_bge_for_rho_and_rhom() is currently only allowed for background estimators of JetMedianBackgroundEstimator type.");
  }
  _common_bge=value;
}
...
void GenericSubtractor::_compute_derivatives(
          const fastjet::FunctionOfPseudoJet<double> &shape,
...
double GenericSubtractor::_component_subtraction(
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    auto_ptr<const fastjet::FunctionOfPseudoJet<double> > shape(shape_ptr->component_shape(i));
