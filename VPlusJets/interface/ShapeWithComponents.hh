...
///
class ShapeWithComponents : public fastjet::FunctionOfPseudoJet<double> {
public:
  //------------------------------------------------------------------
  //------- first, a reminder of the standard functions for a FoPJ ---

  /// the result of computing the shape from a given fastjet::PseudoJet
  ...
  /// n_components().
  virtual std::vector<double> components(const fastjet::PseudoJet &jet) const = 0;
  ...
  virtual double component(int i, const fastjet::PseudoJet &jet) const {
...
inline const fastjet::FunctionOfPseudoJet<double> * ShapeWithComponents::component_shape(unsigned index) const {
