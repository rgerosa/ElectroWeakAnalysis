...
class SimpleGhostRescaler : public fastjet::FunctionOfPseudoJet<fastjet::PseudoJet>{
public:
  ...
  virtual fastjet::PseudoJet result(const fastjet::PseudoJet &jet) const{
    ...
    if (!jet.has_associated_cluster_sequence()){
      ...
      if (!jet.has_pieces())
	throw fastjet::Error("Ghost rescaling can only be performed on jets with an associated ClusterSequence or composite jets (with pieces associated with a Clustersequence)");
    ...
    if (!jet.has_area()){
      throw fastjet::Error("Ghost rescaling can only be applied on jets with an area");
    }

    if (!jet.validated_csab()->has_explicit_ghosts()){
      throw fastjet::Error("Ghost rescaling can only be applied on jets with explicit ghosts");
    }

    fastjet::Selector ghost_selector = fastjet::SelectorIsPureGhost();
    std::vector<fastjet::PseudoJet> rescaled_constituents = jet.constituents();
	...
	rescaled_constituents[i] = fastjet::PtYPhiM(pt, rescaled_constituents[i].rap(), rescaled_constituents[i].phi(), m);
      }
    }
